...
func (w *WatchVulnApp) Run(ctx context.Context) error {
	...
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		case <-ticker.C:
			if isInSleepRange(time.Now(), w.config.SleepRanges) && !*w.config.NoSleep {
...
// isInSleepRange checks if current time is within any of the sleep ranges
func isInSleepRange(now time.Time, ranges []SleepRange) bool {
	if len(ranges) == 0 {
		return false
	}
	current := now.Hour()*60 + now.Minute()
	for _, r := range ranges {
		start, _ := parseHourMinute(r.Start)
		end, _ := parseHourMinute(r.End)
		startMin := start/100*60 + start%100
		endMin := end/100*60 + end%100
		if endMin > startMin {
			if current >= startMin && current < endMin {
				return true
			}
		} else {
			// Cross midnight case like 22:00-06:00
			if current >= startMin || current < endMin {
				return true
			}
		}
...
// parseHourMinute parses "HH:MM" string to integer HHMM
func parseHourMinute(s string) (int, error) {
	if len(s) != 5 || s[2] != ':' {
		return 0, fmt.Errorf("invalid format %s, expected HH:MM", s)
	}
	var h, m int
...
// DB returns the ent database client for external queries.
func (w *WatchVulnApp) DB() *ent.Client { return w.db }

// Config returns the current application configuration.
func (w *WatchVulnApp) Config() *WatchVulnAppConfig { return w.config }

// Grabbers returns the configured vulnerability grabbers.
func (w *WatchVulnApp) Grabbers() []grab.Grabber { return w.grabbers }

// UpdateSources recreates grabbers from current config sources
func (w *WatchVulnApp) UpdateSources() error {
	var grabs []grab.Grabber
	for _, part := range w.config.Sources {
		part = strings.ToLower(strings.TrimSpace(part))
		switch part {
		case "chaitin":
			grabs = append(grabs, grab.NewChaitinCrawler())
		...
		case "venustech":
			grabs = append(grabs, grab.NewVenustechCrawler())
		default:
			return fmt.Errorf("invalid grab source %s", part)
		}
	}
	w.grabbers = grabs
	return nil
...
// TextPusher returns the text pusher for sending test messages.
func (w *WatchVulnApp) TextPusher() push.TextPusher { return w.textPusher }

// RawPusher returns the raw pusher for sending test messages.
func (w *WatchVulnApp) RawPusher() push.RawPusher { return w.rawPusher }

// UpdatePusher updates the pushers with new configuration
func (w *WatchVulnApp) UpdatePusher(pushers []map[string]string) error {
	textPusher, rawPusher, err := CreatePushersFromConfig(pushers)
	if err != nil {
		return err
	}
...
// TriggerCollect runs a single collection cycle and returns the number
// of new vulnerabilities found.
func (w *WatchVulnApp) TriggerCollect(ctx context.Context) (int, error) {
	vulns, err := w.collectUpdate(ctx)
	if err != nil {
		return 0, err
	}
	...
	for _, v := range vulns {
		if w.config.NoFilter || v.Creator.IsValuable(v) {
			dbVuln, err := w.db.VulnInformation.Query().
				Where(vulninformation.Key(v.UniqueKey)).
				First(ctx)
			if err != nil {
				w.log.Errorf("failed to query %s from db %s", v.UniqueKey, err)
				continue
			...
			if !*w.config.NoGithubSearch && v.CVE != "" {
				links, _ := w.FindGithubPoc(ctx, v.CVE)
				if len(links) > 0 {
					v.GithubSearch = grab.MergeUniqueString(v.GithubSearch, links)
				}
			}
			if err := w.pushVuln(v); err != nil {
				...
				_, _ = dbVuln.Update().SetPushed(true).Save(ctx)
				newCount++
			}
		}
	}
...
// TestPush sends a test push notification through configured channels.
func (w *WatchVulnApp) TestPush() error {
