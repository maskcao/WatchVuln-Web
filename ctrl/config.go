...
type WatchVulnAppConfig struct {
	...
	SleepRanges     []SleepRange        `yaml:"sleep_ranges" json:"sleep_ranges"`
	...
	ConfigFile     string        `yaml:"-" json:"-"` // Config file path for persistence
}

type SleepRange struct {
	Start string `yaml:"start" json:"start"` // "00:00" format
	End   string `yaml:"end" json:"end"`     // "07:00" format
}
...
func (c *WatchVulnAppConfig) Init() {
	...
	// Initialize default sleep range if not set and no_sleep is not enabled
	if len(c.SleepRanges) == 0 && !*c.NoSleep {
		c.SleepRanges = []SleepRange{{Start: "00:00", End: "07:00"}}
	}
	if c.DiffMode == nil {
		c.DiffMode = new(bool)
	}
	...
	if c.Console.Listen == "" {
		c.Console.Listen = ":8080"
	}
	if c.Console.Username == "" {
		c.Console.Username = "admin"
	}
	if c.Console.Password == "" {
		c.Console.Password = "admin123"
	}
}
...
func (c *WatchVulnAppConfig) GetPusher() (push.TextPusher, push.RawPusher, error) {
	...
	if len(textPusher) == 0 && len(rawPusher) == 0 {
		golog.Warnf("no pusher configured, vulnerability notifications will be disabled. Configure via web UI or command line.")
	}
...
// CreatePushersFromConfig creates pushers from configuration maps
func CreatePushersFromConfig(pusherConfigs []map[string]string) (push.TextPusher, push.RawPusher, error) {
	var textPusher []push.TextPusher
	var rawPusher []push.RawPusher
...
func (c *WatchVulnAppConfig) Save() error {
	if c.ConfigFile == "" {
		return nil // No config file, skip saving
	}
