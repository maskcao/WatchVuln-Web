...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		&cli.BoolFlag{
			Name:     "console",
			Aliases:  []string{"C"},
			Usage:    "enable web management console",
			Category: "[Console Options]",
		},
		&cli.StringFlag{
			Name:    "listen",
			Aliases: []string{"L"},
			Usage:   "web console listen address",
			Value:   ":8080",
		},
		&cli.StringFlag{
			Name:    "console-username",
			Usage:   "web console login username",
			Value:   "admin",
		},
		&cli.StringFlag{
			Name:    "console-password",
			Usage:   "web console login password",
			Value:   "admin123",
		},
	}
...
func Action(c *cli.Context) error {
	...
	// If config file is not specified, check for default config files
	configPath := c.String("config")
	hasDefaultConfig := false
	if configPath == "" {
		// Try default config files in current directory
		for _, tryPath := range []string{"watchvuln.json", "watchvuln.yaml", "watchvuln.yml"} {
	...
	if hasDefaultConfig {
		c.Set("config", configPath) // Update context for display
		config, err = initConfigFromFile(c)
	} else {
		config, err = initConfigFromCli(c)
		// Set ConfigFile for future saves even in CLI mode, so settings can be persisted
		if config != nil {
			config.ConfigFile = "watchvuln.json"
		}
	}
	if err != nil {
	...
	if config.Console.Enabled {
		webServer, err := web.NewServer(app, config)
		if err != nil {
			return errors.Wrap(err, "failed to create web server")
		}
		go func() {
			log.Infof("web console listening on %s", config.Console.Listen)
			if err := webServer.ListenAndServe(); err != nil {
				log.Errorf("web console error: %s", err)
			}
		}()
	}
...
func initConfigFromFile(c *cli.Context) (*ctrl.WatchVulnAppConfig, error) {
	...
	config.ConfigFile = configFile // Store for later saving
	config.Init()

	// Parse interval from config after Init() sets defaults
	if config.Interval != "" {
		if interval, err := time.ParseDuration(config.Interval); err == nil {
			config.IntervalParsed = interval
		}
	}
...
func initConfigFromCli(c *cli.Context) (*ctrl.WatchVulnAppConfig, error) {
	...
	config := &ctrl.WatchVulnAppConfig{
		...
		Test:            test,
		Console: ctrl.ConsoleConfig{
			Enabled:  c.Bool("console"),
			Listen:   c.String("listen"),
			Username: c.String("console-username"),
			Password: c.String("console-password"),
		},
	}
	return config, nil
}
